...
if ( is_woocommerce_active() ) {
    ...
    update_option('a3rev_woocp_pro_version', '2.0.2');

	define( 'WOOCP_FILE_PATH', dirname( __FILE__ ) );
	define( 'WOOCP_DIR_NAME', basename( WOOCP_FILE_PATH ) );
	define( 'WOOCP_FOLDER', 'woocommerce-compare-products-pro' );
	define(	'WOOCP_NAME', 'woocommerce-compare-products-pro' );
	define( 'WOOCP_URL', YIT_THEME_PLUGINS_URL . '/' . WOOCP_FOLDER );
	...
	WC_Compare_Functions::plugins_loaded();
	 ...
	 */
	if (get_option('a3rev_woocp_just_confirm') == false) woocp_install();    
...
function woocp_install() {
    if ( get_option('a3rev_woocp_pro_version') == '2.0.7' ) return;
